...
W każdym z testów zdefiniowany został jeden parametr konfiguracji, który podlegał wielokrotnym zmianom mającym na celu zaobserwowanie zależności czasu i efektywności pracy
sieci.
\par
Najistotniejszym wnioskiem zaobserwowanym zarówno w momencie tworzenia pracy, jak i procesie tworzenia wyników jest silna zależność między uzyskanymi wynikami a konfiguracją sieci.
...
praca węzłów podlega symulacji przy wykorzystaniu identycznej lokalizacji węzłów zdefiniowanej w konfiguracja domyślna symulowana, jedynie ze zmianą wielkości wiadomości wysyłanej pomiędzy węzłami.
Wielkości te wynosiły 32, 64, 128 i 256 [B].
\par
...
Dla sieci o wielkości X: 800, Y: 800 (konfiguracja domyślna X: 200, Y: 200), uzyskano jedynie 2 rundy dla komunikacji bezpośredniej.
...
\par
Spodziewać się można było, że PEGASIS będący usprawnieniem protokołu LEACH umożliwi uzyskanie dłuższego czasu pracy sieci.
...
W scenariuszach w których odległości pomiędzy węzłami zwiększają się, koszt przekazywania informacji PEGASIS przy pomocy algorytmu `greedy` jest bardzo kosztowny. Każdy kolejny węzeł
odpowiedzialny za przekazanie (`relay`) danych, musi ponieść spory koszt energetyczny.
