...
% \section{Wprowadzenie} % FIXME: zbędne rozdrobnienie podrozdziałów (chyba, że się rozrosną)
...
liczbie aplikacji, zarówno w przemyśle (np. medycynie), jak również w sektorze prywatnym (np. 
inteligentny dom).
...
% TODO: 3-4 zadania o tym, co znajduje się w rozdziale
...
% tutaj można jakieś małe przykłady pokazać, krótko przybliżyć język

Składnia języka jest zbliżona do C/C++ czy Python.

\subsection{Porównanie Go z Python}
% FIXME: jeśli subsection to nie sama lista punktowana
\begin{itemize}
...
% FIXME: albo rozbudować (obrazki?) albo scalić pod jednym tytułem (``narzędzia dodatkowe''?)
...
TODO % TODO w komentarzu się ładnie koloruje w edytorze Kile (polecam)
 * Koszt transmisji
...
LEACH (Low-energy adaptive clustering hierarchy) jest jednym z wielu algortmemów wykorzystywanych w protokołach routingu bezprzewodowej sieci czujnikowej.
W przeciwieństwie do komunikacji bezpośredniej, LEACH jest protokołem w którym występuje hierarchia. Węzły uczestniczące w sieci dzielą się na dwie kategorie:

\begin{itemize}
 \item węzły typu cluster head (CH)
 \item standardowe węzły (non-CH)
\end{itemize}

Standardowe węzły dokonują transmisji danych do najbliższego węzła typu cluster head, które następnie dokonują agregacji zebranch informacji oraz bezpośrednie przekazanie ich
do węzła głównego (sink). Stworzenie środowiska w którym węzły dokonują pośredniczenia informacji, pozwala na ograniczenie kosztów energetycznych transmisji danych przez
 ...
 * Korzyści oraz wyzwania związane z implementacją
...
% \section{Opis} % zbędne section - można od tego zacząć rozdział
...
\begin{lstlisting}
 Uzupełnić w momencie tworzenia wyników do pracy.
 TODO ls folder
\end{lstlisting}
...
%-----------------
\chapter{Przedstawienie i opracowanie wyników} % TODO: a może ``Opracowanie wyników eksperymentów''?
