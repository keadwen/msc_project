...
% Rozbudować wstęp i/lub zamiast section zrobić tu paragraph - dwa poniższe
% podrozdziały są zbyt krótkie
...
% napisałbym o githubie wprost: publiczna i ogólnodostępna dlatego każdy może w
% dowolnym momencie zweryfikować opisane w pracy wyniki. Ponadto można też kod
% pobrać, przeanalizować, porównać z innymi rozwiązaniami. Nie wiem, na jakiej
% licencji to Pan wrzucił ale może też "zmodyfikować i udostępniać dalej".

% W części wstępnej i rozdziałach początkowych powinno być dużo (dużo więcej)
% odniesień do literatury. Tutaj to uzyskać najłatwiej. W zasadzie każdy akapit
% może być obarczony odniesieniem do jakiejś książki lub przeglądowego artykułu
% o WSN.
...
% fajnie by było mieć numerowanie linii kodu

W powyższym przykładzie przedstawione zostało kilka innowacyjnych funkcjonalności języka Go.
% innowacyjnych cech a nie funkcjonalności
% funkcjonalność jest własnością, jest niepoliczalna 
...
\end{itemize}
% To nie jest porównanie. To jest wyliczenie własności.
% Egzekucja oprogramowania? https://pl.wikipedia.org/wiki/Egzekucja
...
\end{itemize}
% Egzekucyjny?... - może być pluton ;) Wykonywalny raczej.
% Wyliczenie przydałoby się zamienić w zwykły tekst.
...
% Ten nawias bez problemu może stać się pełnoprawnym zdaniem.
...
% Teksty angielskie proponuję pisać pismem pochyłym.
...
pracy dyplomowej. 
% Słowo na "f".
% "stworzonym w celach pracy dyplomowej" - słabo. Praca dyplomowa jest opisem.
...
% Tutaj na przykład można dać odnośnik do jakiejś książki a jest ich mnóstwo.
...
% "która jest niezbędna w celu zrozumienia" - słabo. Za ostro, zbyt
% protekcjonistycznie. Trzeba to złagodzić bo w tej formie to może obrazić 
% czytelnika.
...
% Odniesienia do literatury - aż się prosi, żeby tutaj, w tym podrozdziale je
% masowo umieścić.
...
% By móc zrozumiec - do zmiany. Praca dyplomowa to nie podrecznik. Pełni rolę
% sprawozdawczą w pierwszej kolejności. Wyjaśnienie teorii i obserwacji ma być
% dowodem na to, że autor pracy zrozumiał je - jeśli umiemy coś wyjaśnić to
% zazwyczaj to sami rozumiemy.
...
% Przydałoby się napisać, w jakim symulatorze.
...
% W zdecydowanej mierze zależy od odległości, ale nie tylko.
...
% Słowa po angielsku pismem pochyłym proponuję pisać.
...
% 200 x 200 ale czego? Bez miary lub informacji o normalizacji taka
...
% czy nazwa pliku to 'config file'? Bo jeśli nie to nie widzę powodu podawania
...
% Wyżej jest opis a poniżej przykłady. Lepiej byłoby to zmieszać: opis i
% przykład, opis i przykład...
...
% Już nie będę wskazywał użycia 'funkcjonalności' - trzeba całą pracę
% przeszukać i poprawić tekst.
...
% Lista, lista, lista... Dobre w dokumentacji technicznej ale nie w pracy
% dyplomowej. Uważam, że należy poniższą listę (a może listy?) przerobić na tekst.
...
% Tłumaczenie 'metrics' jako 'metryki' to nowomowa i kalka z angielskiego.
% False friend. 'Metrics' to po polsku 'miary', w tym ujęciu. Lepiej jednak to
% po prostu nazwać 'wynikami symulacji', nie kombinować.
...
% Przykład naprawy a może refaktoryzacji :) pracy dyplomowej:
% Węzły sieci są konfigurowane zgodnie z zaimplementowanym protokołem z użyciem
% funkcji Setup. Określenie liczby węzłów sieci jest niezbedne w protokołach
% takich jak LEACH i PEGASIS. W celu zdefiniowania tej wartości wywoływana jest
% funkcja SetNodes. W niektórych protokołach konieczne jest także zdefiniowanie
% liczby klastrów, co jest realizowane funkcją SetClusters.
...
% Poniższa lista jest OK bo rzeczywiście jest prostym wymienieniem parametrów,
% bez ich opisywania - to powinno/zostało zrobione wcześniej.
% Jednak proponuję użyć 'noitemsep'.
\begin{itemize}
...
% Przykładowe ustawienie, czy jedyne jakie zostało użyte w symulacjach? Trzeba
% to sprecyzować.
...
% Poniższe przykłady wymagają szerszego opisu. Czy można 1, 2, czy więcej
% plików? Po co to? Jakie ma konsekwencje? Itd.
...
\begin{itemize}
 \item Wykres ilości aktywnych węzłów w każdej rundzie. % ilości -> liczby
...
% Powyższe zdanie aż prosi się o kontynuację w następnym zdaniu zaczynającym się od "Dlatego...".
...
\end{enumerate}
% Powyższa lista może zostać, chociaż nie wiem na ile te wskazówki są
% wystarczające.
% 'nowy protokołu'
...
% oprogramowania omówionego wcześniej.
...
% Nagła zmiana stylu tekstu - 'posiadamy'.
...
% Ta lista niczego nie mówi, żadnej treści nie ma. Zamiast tego powinien być
% opis tej listy, co zawiera, dlaczego tak a nie inaczej...
...
% Poniższy listing jest OK, ale można to opisać lepiej. W połączeniu z opisem
% listy konfigów będzie OK.
...
% Tabela ma za dużo białej przestrzeni. Trzeba ją przeorganizować. Pierwsze
% dwie kolumny de facto są nagłówkiem. Całość pewnie da się zrobić w poziomie -
% chyba o tym Panu pisałem.
...
% Nad tą tabelą tak samo jak nad poprzednią trzeba trochę popracować.
\begin{longtable}{*{4}{c}}
\toprule
\multicolumn{4}{c}{Uzyskane rezultaty} \\
Protokół	& Wielkość wiadomości (B)	& Kwantyl	& Wartość \\
...
% Jak wcześniej - lista jest bez sensu, jeśli nie wiadomo co w plikach jest.
% Lepiej je opisać, napisać w jakim są katalogu itd...
\begin{lstlisting}
...
Uruchomienie oprogramowania:
% Szablon - więc i uwagi takie jak wyżej. I tak aż do końca rozdziału. 
% Rinse and repeat.
...
% W niniejszej pracy przedstawiony został autorski projekt oprogramowania do
% symulacji bezprzewodowych sieci czujnikowych. 
...
% Denerwujące słowo na "f"? ;P
% O konsultowaniu nie piszemy. Ja to napiszę w opinii.
...
% Tu bym się pochwalił, że to na githubie i że każdy może obejrzeć i uruchomić
% sam. Otwarty kod krytyki się nie boi ;)
\par
% \par? Dlaczego nie pusta linia?
...
% Wyników nie 'tworzymy'. Można je na przykład 'zebrać', 'uzyskać'. 'Tworzenie'
% pasuje do plików z wynikami symulacji - te pliki rzeczywiście są 'tworzone'
% przez oprogramowanie. Subtelna ale istotna różnica.
...
% Przypadek szczególny. Podsumowanie powinno uogólniać, wyciągać spójne
% wnioski.
\par
...
% To powyżej jest bardzo fajne. Trzeba to uwypuklić. W podsumowaniu można sobie
% pozwolić na większą swobodę języka. Tak jak wcześniej należy przedstawiać
% podstawy wiedzy, fakty, obiektywne prawdy tak tutaj można skomentować
% uzyskane rezultaty. Dlatego sformułowania typu 'zaskakujący', 'nietypowy',
% 'niespodziewany' są w porządku.

% W podsumowaniu można też napisać w pierwszej osobie, czego nowego udało się
% nauczyć, jakie doświadczenie zdobyć, jaką dotychczasową wiedzę i zdolności
% potwierdzić lub ugruntować.

% Można też napisać, co dałoby się jeszcze dodać do powstałego rozwiązania,
% jakie ma ono braki (ale nie błędy), jakie zagadnienia byłyby zdaniem autora
% warte przeprowadzenia dalszych badań.
...
% Normalnie recenzja i opinia są drukowane osobno - nie pojawiają się w PDF
% dostarczanym przez dyplomanta i nie są oprawiane.
